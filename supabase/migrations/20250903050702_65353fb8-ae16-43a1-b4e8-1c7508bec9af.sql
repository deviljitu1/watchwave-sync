-- Allow participants to update playback state while keeping other fields protected
-- 1) Add RLS policy permitting participants to update rooms
CREATE POLICY IF NOT EXISTS "Participants can update playback state"
ON public.rooms
FOR UPDATE
USING (
  auth.uid() IN (
...
-- 2) Create a trigger to enforce that non-hosts only change playback fields
